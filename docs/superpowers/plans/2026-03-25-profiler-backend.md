...
```typescript
import { mkdtempSync, rmSync, readFileSync, existsSync, mkdirSync, writeFileSync, lstatSync } from 'fs';
```
...
import { CreateProfileBodySchema, UpdateProfileBodySchema, SAFE_NAME_PATTERN } from '@claudeui/shared';
...
import { readdir, readFile, copyFile, mkdir, access, stat } from 'fs/promises';
import path from 'path';
import type { Profile } from '@claudeui/shared';
  ...
  async import(sourceDir: string): Promise<{ imported: number; skipped: number; errors: string[] }> {
    ...
    try {
      ...
      for (const f of files.filter(f => f.endsWith('.md'))) {
        ...
        try { await access(dest); skipped++; } catch {
          try { await copyFile(path.join(agentsSrc, f), dest); imported++; } catch (e: any) { errors.push(`agent ${f}: ${e.message}`); }
        }
      }
    } catch { /* no agents dir */ }
    ...
    try {
      ...
      for (const entry of entries) {
        ...
        try { await access(path.join(srcSkill, 'SKILL.md')); } catch { continue; }
        const destSkill = path.join(skillsDest, entry);
        try { await access(destSkill); skipped++; } catch {
          try {
            ...
            imported++;
          } catch (e: any) { errors.push(`skill ${entry}: ${e.message}`); }
        }
      }
    } catch { /* no skills dir */ }
    ...
    try {
      ...
      for (const f of files.filter(f => f.endsWith('.md'))) {
        ...
        try { await access(dest); skipped++; } catch {
          try { await copyFile(path.join(cmdsSrc, f), dest); imported++; } catch (e: any) { errors.push(`command ${f}: ${e.message}`); }
        }
      }
