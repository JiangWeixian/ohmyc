...
import type { InstalledPlugin, Marketplace, PluginInstall, PluginManifest } from '@claudeui/shared';
...
async function fetchPlugins(): Promise<PluginInventoryItem[]> {
  ...
  return Array.isArray(data.plugins) ? data.plugins.map(normalizePlugin) : [];
}
