      ...
      {keys.length > 0 ? (
        <div className="flex flex-wrap gap-1.5">
          {keys.map((key) => (
            <span
              key={key}
              className="inline-block rounded bg-[#5E6AD2]/8 px-2 py-0.5 text-[12px] font-mono text-[var(--text-secondary)] border border-[#5E6AD2]/15"
            >
              {key}
            </span>
          ))}
...
export function ProfileCard({ profile, isActive, activeProfileName, onActivate, onDeactivate, onDelete, onEdit }: ProfileCardProps) {
  ...
  }, [profile.name, preflightMut, onActivate]);
        ...
        <div className="flex flex-col gap-4 lg:w-[35%]">
          <div>
            <div className="flex items-center gap-2.5">
              ...
              {isActive && (
                <span className="bg-[#5E6AD2]/15 text-[#5E6AD2] text-[11px] font-medium px-2 py-0.5 rounded">
                  Active
                </span>
              )}
            </div>
