...
import { Plus, Trash2, Edit2, Loader2, ChevronDown, Info } from 'lucide-react';
import { motion, AnimatePresence } from 'framer-motion';
import { useStoreAgents, useStoreSkills, useStoreCommands, useDeleteStoreAgent, useDeleteStoreSkill, useDeleteStoreCommand, useStoreModelConfigs, useDeleteStoreModelConfig } from '../../hooks/useStore';
...
export function StoreComponentList({ category }: StoreComponentListProps) {
  const [editing, setEditing] = useState<{ category: 'agents' | 'skills' | 'commands' | 'model-configs'; name?: string } | null>(null);
  const [deleteTarget, setDeleteTarget] = useState<{ category: 'agents' | 'skills' | 'commands' | 'model-configs'; name: string; referencedBy: string[] } | null>(null);
  ...
  const [typeFilter, setTypeFilter] = useState<'all' | 'agents' | 'skills' | 'commands' | 'model-configs'>(
  ...
  for (const p of allProfiles) {
    ...
    for (const ref of p.skills) {
      const key = `skills:${ref}`;
      const existing = referencedByMap.get(key) ?? [];
      existing.push(p.name);
      referencedByMap.set(key, existing);
    }
    for (const ref of p.commands) {
      const key = `commands:${ref}`;
      const existing = referencedByMap.get(key) ?? [];
      existing.push(p.name);
      referencedByMap.set(key, existing);
    }
  ...
  const handleDelete = (itemCategory: 'agents' | 'skills' | 'commands' | 'model-configs', name: string) => {
    const deleteMut = itemCategory === 'agents' ? deleteAgentMut : itemCategory === 'skills' ? deleteSkillMut : itemCategory === 'commands' ? deleteCommandMut : deleteModelConfigMut;
    deleteMut.mutate({ name, force: false }, {
      onSuccess: () => setDeleteTarget(null),
      onError: (err: any) => {
        const refs = err.data?.referencedBy;
        if (refs) {
          setDeleteTarget({ category: itemCategory, name, referencedBy: refs });
        }
      },
    });
  };
  ...
  const handleForceDelete = () => {
    ...
    const deleteMut =
      deleteTarget.category === 'agents'
        ? deleteAgentMut
        : deleteTarget.category === 'skills'
          ? deleteSkillMut
          : deleteTarget.category === 'commands'
            ? deleteCommandMut
