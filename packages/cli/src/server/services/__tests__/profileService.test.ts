...
import { mkdtempSync, rmSync, readFileSync, existsSync, mkdirSync, writeFileSync, lstatSync, readlinkSync, chmodSync } from 'fs';
import path from 'path';
import os from 'os';
import { ProfileService } from '../profileService';
import { ActivationBlockedError } from '../profileService';
...
describe('ProfileService', () => {
  ...
  describe('activate()', () => {
    ...
    it('writes .active with absolute path but list() returns bare name', async () => {
      ...
      const activeFile = readFileSync(path.join(tmpDir, 'profiles', '.active'), 'utf-8').trim();
  ...
  describe('activate() - transactional', () => {
    ...
    it('no filesystem changes when activation is blocked by missing components', async () => {
      const settingsBefore = readFileSync(path.join(tmpDir, 'settings.json'), 'utf-8');
      await service.create({ name: 'test', agents: ['nonexistent'], settings: { effort: 'high' } });

      await expect(service.activate('test')).rejects.toThrow();

      // No settings changes
      const settingsAfter = readFileSync(path.join(tmpDir, 'settings.json'), 'utf-8');
  ...
  describe('deactivate() - model config env var restoration', () => {
    ...
    it('restores env field to pre-activation state on deactivation', async () => {
      ...
      const afterActivate = JSON.parse(readFileSync(path.join(tmpDir, 'settings.json'), 'utf-8'));
    ...
    it('switching from profile with model config to profile without restores env vars', async () => {
      ...
      await service.activate('profile-a');
      const afterA = JSON.parse(readFileSync(path.join(tmpDir, 'settings.json'), 'utf-8'));
      expect(afterA.env.ANTHROPIC_AUTH_TOKEN).toBe('sk-key-a-1111');

      await service.activate('profile-b');
      const afterB = JSON.parse(readFileSync(path.join(tmpDir, 'settings.json'), 'utf-8'));
