import { readdir, readFile, writeFile, mkdir, rm, access, stat } from 'fs/promises';
...
export class SkillService {
  ...
  async create(frontmatter: SkillFrontmatter, content: string): Promise<Skill> {
    ...
    await writeFile(path.join(skillDir, SKILL_FILE), raw, 'utf-8');
  ...
  async update(
    ...
    await writeFile(path.join(this.skillsDir, name, SKILL_FILE), raw, 'utf-8');
