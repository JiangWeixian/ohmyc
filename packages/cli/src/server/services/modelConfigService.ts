...
function safeConfigPath(configsDir: string, name: string): string | null {
  if (!MODEL_CONFIG_NAME_RE.test(name)) return null;
  if (name.includes('..')) return null;
  const filePath = path.join(configsDir, `${name}.json`);
  const resolved = path.resolve(filePath);
  if (!resolved.startsWith(path.resolve(configsDir) + path.sep) && resolved !== path.resolve(configsDir)) return null;
  return filePath;
}
...
export class ModelConfigService {
  ...
  async list(): Promise<ModelConfig[]> {
    ...
    for (const filename of files.filter(f => (f as string).endsWith('.json')).sort()) {
      try {
        const raw = await readFile(path.join(this.configsDir, filename as string), 'utf-8');
