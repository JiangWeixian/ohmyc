...
import type { InstalledPlugin, PluginInstall, PluginManifest, Marketplace } from '@claudeui/shared';
...
export class PluginService {
  ...
  private async scanComponents(installPath: string): Promise<PluginComponents> {
    const components: PluginComponents = {
      ...
      hooks: null, mcpServers: null, lspServers: null,
    };
    ...
    try {
      ...
      components.agents = files.filter(f => f.endsWith('.md')).map(f => f.replace(/\.md$/, '')).sort();
    } catch { /* no agents dir */ }
    ...
    try {
      ...
      components.commands = files.filter(f => f.endsWith('.md')).map(f => f.replace(/\.md$/, '')).sort();
