...
| 2026-06-25 | Missing monitor store uses a full-app onboarding gate | When the local monitor store is unavailable, the main app should show a headerless setup gate before Explorer routes instead of rendering a broken or empty Timeline. The gate is a product unavailable state: no Navigation Island, no app chrome, manual plugin repo link plus Retry, and no database path shown. The visual subject uses the frosted-ivory-lamplit Atropos computer with real layered shell/keyboard depth, reduced-motion fallback, and low-contrast Letter Glitch ambience |
